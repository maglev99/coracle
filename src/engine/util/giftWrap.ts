...
export const wrap = (authorPrivkey, recipientPubkey, wrapperPrivkey, event, tags = []) => {
  ...
  const wrap = createWrap(authorPrivkey, recipientPubkey, seal, tags)

  return wrap
}

export const unwrap = (recipientPrivkey, wrap) => {
  const sealKey = getConversationKey(recipientPrivkey, gift.pubkey)
  const seal = JSON.parse(decrypt(sealKey, JSON.parse(gift.content)))
